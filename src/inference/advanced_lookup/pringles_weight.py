...
pose_index_dict = {
   ...
   ("palm", "light"): [],
   ("palm", "heavy"): [39, 40],
   ("tripod", "empty"): [9, 34, 54],
   ("tripod", "light"): [8, 11, 13],
...
if __name__ == "__main__":
    ...
    event_dict = {"q":stop_event, "y":start_event, "e":end_event}
    listen_keyboard(event_dict)
    ####################################
    
    for grasp_type in ["up", "tip", "palm", "tripod", "lay"]:
        for grasp_obj_type in ["empty", "light", "heavy"]:
            pose_index_list = pose_index_dict[(grasp_type, grasp_obj_type)]
            grasp_obj_name = "pringles" + ("" if grasp_obj_type == "empty" else ("_" + grasp_obj_type))
            
            for pose_index in pose_index_list[:2]:
                for i in range(num_try):
                    info = {
                        "grasp_type":grasp_type,
                        "grasp_obj_type":grasp_obj_name,
                        "pose_index":pose_index,
                        "index":i,
                        "success":True
                        }
                    
                    save_path = os.path.join("inference", "weight_test", args.object, grasp_type, grasp_obj_type, str(pose_index), str(i))
                    ...
                    print(f"press y after fixing object position + current grasp type {grasp_type} {grasp_obj_type} {pose_index} {i}")        
                    while not start_event.is_set() and not stop_event.is_set() and not end_event.is_set():
                        time.sleep(0.1)
                    ...
                    _, traj, hand_traj = get_traj(grasp_obj_name, hand_name, start_pos.copy(), pick_6D.copy(), place_6D.copy(), index=str(pose_index))
                    ...
                    for i in range(len(traj)):
                        ...
                        if stop_event.is_set() or end_event.is_set():
                            info["success"] = False
                            break
