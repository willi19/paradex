...
def create_grid_plane(size=1.0, step=0.1):
    lines = []
    points = []
...
class simpleViewer(object):
    ...
    def add_plane(self, size=1, step=0.1, transform=None):
        if self.plane is None:
            grid_plane = create_grid_plane(size=3)
            if transform is not None:
                grid_plane = grid_plane.transform(transform)
            self.plane = grid_plane
            self.main_vis.add_geometry(self.plane, reset_bounding_box=self.reset_flag)
        return
