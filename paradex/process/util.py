...
def process_video_list(video_dir, data, process_frame, logger=None):
    ...
    while True:
        ...
        print(time.time() - process_start_time, "required time per process")
        fid += 1
...
def merge(root_dir, logger=None, overwrite=False):
    # Download video
    download_root_dir = root_dir.replace(shared_dir, download_dir)
    ...
    serial_list = []
    for video_name in os.listdir(os.path.join(root_dir, "overlay")):
        if logger:
            logger.update_status(f"Merge : Downloading video {os.path.join(root_dir, 'overlay', video_name)}")
        copy_file(os.path.join(root_dir, "overlay", video_name), os.path.join(download_video_dir, video_name))
        serial_list.append(video_name.split(".")[0])
    serial_list.sort()
