...
for demo_name in ["7"]:#demo_name_list:
    for type in ["pick", "place"]:
        # if os.path.exists(os.path.join(shared_dir, f"capture/lookup/{obj_name}/{demo_name}/pick.mp4")):
        #     continue
        ...
        wrist_pos = np.load(os.path.join(demo_path, str(demo_name), f"{type}_action.npy"))
        obj_pos = np.load(os.path.join(demo_path, str(demo_name), f"{type}_obj_T.npy"))
        obj_pose = obj_pos[0].copy() if type == "pick" else obj_pos[-1].copy()
...
while True:
    ...
    while cur_env_idx < len(env_list) and len(cur_idx) < max_env_num:
        ...
        sim.set_videopath(env_name, os.path.join(download_dir, "capture","lookup", obj_name))
