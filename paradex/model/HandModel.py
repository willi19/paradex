...
from paradex.model.utils.rot6d import *
from paradex.model.utils.utils_math import *
...
class HandModel:
    def __init__(self, robot_name, urdf_filename, mesh_path,
        ...
        # prepare model
        with open(urdf_filename, "rb") as f:
            self.robot = pk.build_chain_from_urdf(f.read()).to(dtype=torch.float, device=self.device)
    ...
    def update_kinematics(self, q):
        self.global_translation = torch.zeros([q.shape[0], 3], device=self.device)        
        self.global_rotation = torch.eye(3, device=self.device).unsqueeze(0).repeat(q.shape[0], 1, 1)

        self.current_status = self.robot.forward_kinematics(q)
