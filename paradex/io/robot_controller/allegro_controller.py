...
class AllegroController:
    def __init__(self, addr=None, **_):
        ...
        # Signals
        self.exit = Event()
        self.connection_event = Event()  # set after first joint state received
        self.lock = Lock()
    ...
    def get_data(self):
        """Return current joint state and target action."""
        with self.lock:
            qpos = None
            if self.current_joint_pose != DEFAULT_VAL:
                qpos = np.asarray(self.current_joint_pose.position)
