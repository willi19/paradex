...
try:
    ...
    while True:
        ...
        current_number = get_framenumber(cur_state, cur_numinput, cur_tg_frame, args.static) 
        if current_number >= cam_numb_thres:   
            print(f"Processing start with frame {cur_tg_frame}")
            time.sleep(0.1) # wait for all input to be stored            
            ...
            matchingset_list = [s for s in matchingset_list if s is not None and len(s.set) >= 3]
