...
import numpy as np
from paradex.geometry.triangulate import triangulate, ransac_triangulation
...
if __name__ == "__main__":
    name_list = ['20250720_024117', '20250720_025258', '20250720_031957', '20250720_033429', '20250720_033814']
    for name in name_list:
        intrinsics, extrinsics = load_current_camparam_fixed(name)
        ...
        for index in index_list:
            ...
            for serial_num, v in proj_err.items():
                if len(v) == 0:
                    continue
                if np.max(v) > 2:
                    frame = image_dict[serial_num]
                    os.makedirs(f"debug/{name}_fix/{index}", exist_ok=True)
                    cv2.imwrite(f"debug/{name}_fix/{index}/{serial_num}.png", cv2.resize(frame, (int(frame.shape[1]), int(frame.shape[0]))))
        
        print(name, np.mean(proj_err_tot))