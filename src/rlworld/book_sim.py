...
if __name__ == "__main__":
    ...
    # sensors["camera"].start(f"shared_data/inference/register")
    # sensors["camera"].end()
    
    register_dict = {}
    for img_name in os.listdir(f"{shared_dir}/inference/register"):
        serial_num = img_name.split(".")[0]
        register_dict[serial_num] = cv2.imread(f"{shared_dir}/inference/register/{img_name}")
