...
def grid_image(img_list, frame, input_folder, output_folder, grid_shape = (4,6), size=(1080, 1080)):
    rows, cols = grid_shape
    h, w = img_list[0].shape[:2]
    os.makedirs(os.path.join(input_folder, output_folder), exist_ok = True)
    grid_image = np.zeros((rows * h, cols * w, 3), dtype = np.uint8)    
    for idx, img in enumerate(img_list):
        row, col = divmod(idx, cols)
        grid_image[row*h:(row+1)*h, col*w:(col+1)*w] = img
    grid_image = cv2.resize(grid_image, size)
    cv2.imwrite(os.path.join(input_folder, output_folder, frame), grid_image)

def show_res_cv2(cam_id, image, masks, scores, alpha = 0.2):
    if image is not None:
        mask = (masks != 0).astype(np.uint8)
        overlay = image.copy()
        mask_colored = np.zeros_like(image, dtype=np.uint8)
        mask_colored[:, :, 2] = 128  # 빨간색 채널 강조
        # 마스크 적용
        mask_bool = mask.astype(bool)  # 마스크를 이진화
        overlay[mask_bool] = cv2.addWeighted(image, 1 - alpha, mask_colored, alpha, 0)[mask_bool]

        # 마스크 경계선 그리기
        contours, _ = cv2.findContours(mask.astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        cv2.drawContours(overlay, contours, -1, (0, 0, 0), 1)  # 초록색 테두리

        overlay = cv2.cvtColor(overlay, cv2.COLOR_RGB2BGR)
        overlay  = cv2.resize(overlay, (384, 512), interpolation = cv2.INTER_AREA)
    else:
        overlay = np.zeros((384, 512, 3), dtype=np.uint8)
