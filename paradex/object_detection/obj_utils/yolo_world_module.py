...
def check_mask(mask):
    ...
    if len(xs)>0 and len(ys)>0:
        x_L = (xs.max()-xs.min())
        y_L = (ys.max()-ys.min())
        x_percentage = x_L/mask.shape[1]
        y_percentage = y_L/mask.shape[0]

        # check overdetermined mask
        if x_percentage < 0.7 and y_percentage < 0.7:
            return True
    return False


class YOLO_MODULE:
    def __init__(self, model_id='yolo_world/v2-l', categories:str=None, device=DEVICE, use_sam=False):
        if use_sam:
