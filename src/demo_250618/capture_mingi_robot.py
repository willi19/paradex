...
def main_loop(yolo_module, hand_module):
    ...
    while True:
        ...
        cur_cnt = 0
        arm_controller.set_target_action(
                arm_traj[current_idx]
        )
        
        current_arm_angles = arm_controller.arm_state_hist[arm_controller.arm_cnt]
        np.save(os.path.join("/home/temp_id/shared_data/demo_250618/pringles/demo_250618_optim/final", 'arm_state.npy'), current_arm_angles)
        
        hand_controller.set_target_action(
                hand_traj[current_idx]
            )
        ...
        if cur_cnt == len(serial_list):
            ...
            # os.makedirs(os.path.join(shared_dir, "demo_250618", "pringles", "demo_250618", "final", 'arm'), exist_ok=True)
            # np.save(os.path.join("/home/temp_id/shared_data/demo_250618/pringles/demo_250618_optim/final", 'state.npy'), current_arm_angles)
            ...
            for cam_id in org_scene.cam_ids:
                ...
                if rgb_img is None or rgb_img.size == 0:
                    rgb_img = np.zeros((1536, 2048, 3), dtype=np.uint8)
            ...
            try:
                ...
                grid_img = make_grid_image_np(np.array(results_img), 4, 6)
                imshow_obj.set_data(grid_img[..., ::-1])
                fig.canvas.draw()
                fig.canvas.flush_events()
            except ValueError as e:
                print(f"object not detected: {e}")
...
# arm.set_mode(2)
# arm.set_state(0)
arm_controller = XArmController(None)
hand_controller = AllegroController(None)


hand_controller.set_homepose(hand_traj[0])
hand_controller.home_robot()
            
arm_controller.set_homepose(arm_traj[0])
