...
runs cv2.calibrateCamera with per-frame masking of NaN entries, and writes
the result to ~/shared_data/intrinsic/<serial>/param/<timestamp>.json
matching the pre-existing format ({K, distortion, RMS_error, width, height}).
"""
...
def main():
    ...
    parser.add_argument("--serials", nargs="+", required=True,
                        help="one or more camera serial numbers")
