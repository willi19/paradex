...
ik_data = np.load("data/ik.npy", allow_pickle=True).item()
sim = 
...
#             tz_dict[tz] = []
#         tz_dict[tz].append((tx,ty,succ))
#         # 시각화: tz별 이미지
#     for tz in sorted(tz_dict.keys()):
#         points = tz_dict[tz]
#         plt.figure(figsize=(3,6))
#         for tx, ty, success in points:
#             # 스케일링 (필요 시, 예: 정규화 등)
#             x = tx * 100  # 예: 이미지 정중앙 기준
#             y = ty * 100

#             plt.scatter(x, y, color='green' if success else 'red', s=4)
