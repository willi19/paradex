...
for step in range(len(pick_position)):
    ...
    #     print(f"Pick step {step}, {idx}/{len(tot_traj)}")
    ...
    #     for i in range(traj_cnt):
    #         if quit_event.is_set():
    #             break

    #         if idx+i < len(tot_traj):
    #             arm_controller.set_action(tot_traj[idx+i, :6])
    #             hand_controller.set_target_action(tot_traj[idx+i, 6:])
    #             time.sleep(0.03)
