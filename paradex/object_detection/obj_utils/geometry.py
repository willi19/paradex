...
def project_3d_to_2d(points3d, projections, same_dim=False):
    # points3d : NX3
    # projections: MX3X4

    points_3d_homo = np.concatenate((points3d, np.ones((points3d.shape[0],1))), axis=-1)
    if same_dim:
        points_2d_homo = np.einsum('md,mjd->mj',points_3d_homo, projections)
    else:
...
def project_3d_to_2d_tensor(points3d_t, projections_t, same_dim=False):
    # points3d : NX3
    # projections: MX3X4

    points_3d_homo = torch.concat((points3d_t, torch.ones((points3d_t.shape[0],1), device=points3d_t.device)), axis=-1)
    if same_dim:
        points_2d_homo = torch.einsum('md,mjd->mj',points_3d_homo, projections_t)
    else:
...
import os
def get_visualhull_ctr(scene, resolution=0.01, mask_dict = None): # mask_dict 
    camera_center_arr = np.stack([scene.camera_centers[cam_id] for cam_id in scene.camera_centers])
    voxel_range = np.stack([np.min(camera_center_arr, axis=0), np.max(camera_center_arr, axis=0)])
    grid = generate_voxel_grid(voxel_range, resolution=resolution)
    ...
    in_mask_points = grid[in_mask_number>in_mask_number.max()*0.6]
...
def get_bbox_center(scene, tg_cams=None, mask_dict = None):
    A = []
    tg_cams = scene.cam_ids if tg_cams is None else tg_cams
    for cam_id in tg_cams:    
        if mask_dict is not None and cam_id in mask_dict:
            mask = mask_dict[cam_id]
        else:
...
#     """
#     Converts rotation matrices to 6D rotation representation (Zhou et al. 2019).
#     Keeps the first two columns of the rotation matrix.
...
#     rot_6d = matrix[..., :3, :2].reshape(batch_shape + (6,))
#     return rot_6d


def rotation_6d_to_matrix_np(r6d):
    # r6d: (6,) numpy array
    a1 = r6d[:3]
    a2 = r6d[3:]
...
def normalize_rotation_matrix(R_mat):
...
def cluster_rotations(rotation_matrices, distance_threshold=0.3):
    from sklearn.cluster import AgglomerativeClustering
    """Filter noise, cluster similar rotations, and return cluster representatives."""
    
    # Step 1: Normalize all rotation matrices
    rotations = [normalize_rotation_matrix(R) for R in rotation_matrices]

    # Step 2: Compute mean distance of each rotation to all others (for filtering)
    distances = []
    for i in range(len(rotations)):
        d = [rotation_distance(rotations[i], r) for j, r in enumerate(rotations) if i != j]
        distances.append(np.mean(d))
    
    # Step 3: Noise filtering
    threshold = np.percentile(distances, 80)  # top 20% worst are considered noisy
    filtered_rotations = [r for i, r in enumerate(rotations) if distances[i] <= threshold]

    if len(filtered_rotations) == 0:
        return []  # All filtered out

    quats = [R.from_matrix(r).as_quat() for r in filtered_rotations]

    # Flip sign to ensure consistency (all w > 0)
    quats = [q if q[0] >= 0 else -q for q in quats]

    # Cluster in quaternion space
    clustering = AgglomerativeClustering(n_clusters=None,
                                        distance_threshold=distance_threshold,
                                        linkage='average')
    labels = clustering.fit_predict(quats)
    print(labels)

    # Step 6: Compute representative for each cluster
    representatives = []
    for label in np.unique(labels):
        group = [filtered_rotations[i] for i in range(len(filtered_rotations)) if labels[i] == label]
        group_vecs = [R.from_matrix(r).as_rotvec() for r in group]
        mean_vec = np.mean(group_vecs, axis=0)
        representatives.append(R.from_rotvec(mean_vec).as_matrix())
