...
class RobotGUIController:
    def __init__(self, robot_controller, hand_controller, 
            predefined_poses=None, grasp_pose=None,
            approach_traj=None, aftergrasp_traj=None, lift_distance=100.0, place_distance=40.0):
        """
        Args:
            ...
            grasp_pose: dict with 'start', 'pregrasp', 'grasp', 'squeezed' keys containing full poses [N, 12] (arm 6 + hand 6)
            approach_traj: numpy array, approach trajectory [N, 12] (xarm 6 + hand 6)
            aftergrasp_traj: numpy array, aftergrasp trajectory [N, 12] (xarm 6 + hand 6)
            lift_distance: float, lift distance in mm (default: 100mm, ignored if aftergrasp_traj provided)
        ...
        self.aftergrasp_traj = aftergrasp_traj
        ...
        self.aftergrasp_progress = 0.0

        # Height trackers
        self.lift_start_z = None
        self.lift_start_pose = None
    ...
    def _build_gui(self):
        # Build window title with trajectory info
        title_text = "Robot GUI Controller"
        if self.approach_traj is not None:
            title_text += f" | Approach: {len(self.approach_traj)} steps"
        if self.aftergrasp_traj is not None:
            title_text += f" | Aftergrasp: {len(self.aftergrasp_traj)} steps"

        self.root.title(title_text)
        self.root.geometry("900x850")

        # Header with trajectory status
        header_frame = tk.Frame(self.root)
        header_frame.pack(pady=5)

        title = tk.Label(header_frame, text="Robot Controller", font=("Arial", 16, "bold"))
        title.pack()

        # Trajectory info label
        traj_info_text = ""
        if self.approach_traj is not None:
            traj_info_text += f"Approach: {len(self.approach_traj)} waypoints | "
        if self.aftergrasp_traj is not None:
            traj_info_text += f"Aftergrasp: {len(self.aftergrasp_traj)} waypoints"
        else:
            traj_info_text += f"Lift: {self.lift_distance}mm"

        if traj_info_text:
            traj_info_label = tk.Label(header_frame, text=traj_info_text,
                                      font=("Arial", 9), fg="gray")
            traj_info_label.pack()
    ...
    def _build_task_frame(self):
        """Approach, Grasp, Squeeze, Aftergrasp/Lift 버튼 프레임"""
        task_frame = tk.LabelFrame(self.root, text="Task Execution", font=("Arial", 12))
        task_frame.pack(pady=10, padx=10, fill="both")

        # Approach 버튼
        approach_btn = tk.Button(task_frame, text="Approach", width=12, bg="skyblue",
                                font=("Arial", 10, "bold"))
        btn_name = "approach"
        self.button_states[btn_name] = False
        ...
        # Grasp 버튼
        grasp_btn = tk.Button(task_frame, text="Grasp", width=12, bg="lightcoral",
                             font=("Arial", 10, "bold"))
        btn_name = "grasp"
        self.button_states[btn_name] = False
        ...
        # Squeeze 버튼
        squeeze_btn = tk.Button(task_frame, text="Squeeze", width=12, bg="salmon",
                               font=("Arial", 10, "bold"))
        btn_name = "squeeze"
        self.button_states[btn_name] = False
        squeeze_btn.bind('<ButtonPress-1>', lambda e, name=btn_name: self._on_button_press(name))
        squeeze_btn.bind('<ButtonRelease-1>', lambda e, name=btn_name: self._on_button_release(name))
        squeeze_btn.pack(side=tk.LEFT, padx=5, pady=5)

        # Aftergrasp/Lift 버튼
        aftergrasp_text = "Aftergrasp" if self.aftergrasp_traj is not None else "Lift"
        lift_btn = tk.Button(task_frame, text=aftergrasp_text, width=12, bg="lightgreen",
                            font=("Arial", 10, "bold"))
        btn_name = "lift"
        self.button_states[btn_name] = False
    ...
    def _execute_approach(self):
        ...
        if distance > 0.01:  # Not reached yet
            ...
            progress_pct = (self.approach_progress / len(self.approach_traj)) * 100
            if self.approach_progress >= len(self.approach_traj):
                print(f">>> Approach trajectory complete! (100%)")
            else:
                if target_idx % 5 == 0:  # Print every 5 waypoints
                    print(f"Approach progress: {target_idx}/{len(self.approach_traj)} ({progress_pct:.1f}%)")

    def _execute_grasp(self):
        """Grasp sequence 실행 - pregrasp까지만 (누르고 있는 동안 자동 진행)"""
        if self.predefined_hand_traj is None:
            print("No grasp trajectory loaded!")
            return

        # Extract full poses (12 DOF: arm 6 + hand 6)
        start_pose = self.predefined_hand_traj['start'][0,:]  # First row of NX12
        pregrasp_pose = self.predefined_hand_traj['pregrasp'][0,:]
        grasp_pose = self.predefined_hand_traj['grasp'][0,:]

        if self.hand_grasp_progress < 2.0:
            self.hand_grasp_progress = min(2.0, self.hand_grasp_progress + self.hand_grasp_speed)

            # Determine current stage and interpolate (full pose)
            progress = self.hand_grasp_progress
            if progress <= 1.0:
                # Stage 1: start -> pregrasp
                current_full_pose = start_pose + (pregrasp_pose - start_pose) * progress
            else:
                # Stage 2: pregrasp -> grasp
                stage_progress = progress - 1.0
                current_full_pose = pregrasp_pose + (grasp_pose - pregrasp_pose) * stage_progress

            # Move arm and hand separately
            current_qpos = self.robot.get_data()['qpos'].copy()
            arm_target = current_full_pose[:6]
            hand_target = current_full_pose[6:]

            # Calculate arm delta
            arm_delta = arm_target - current_qpos
            arm_distance = np.linalg.norm(arm_delta)
            if arm_distance > 0.01:
                # Limit arm movement speed
                arm_delta_norm = np.linalg.norm(arm_delta)
                if arm_delta_norm > 0:
                    arm_delta = arm_delta / arm_delta_norm * min(self.joint_delta, arm_delta_norm)
                new_arm_qpos = current_qpos + arm_delta
                self.robot.move(new_arm_qpos, is_servo=True)

            self.hand.move(hand_target)
            self.hand_progress_label.config(text=self._get_progress_text())
        else:
            print("Grasp sequence complete (at grasp pose)!")

    def _execute_squeeze(self):
        """Squeeze sequence 실행 - grasp -> squeezed (누르고 있는 동안 자동 진행)"""
        if self.predefined_hand_traj is None:
            print("No grasp trajectory loaded!")
            return

        # Extract full poses (12 DOF: arm 6 + hand 6)
        grasp_pose = self.predefined_hand_traj['grasp'][0,:]  # First row of NX12
        squeezed_pose = self.predefined_hand_traj['squeezed'][0,:]

        # If not at grasp yet, move there first
        if self.hand_grasp_progress < 2.0:
            self.hand_grasp_progress = 2.0
            # Move to grasp pose (arm + hand)
            self.robot.move(grasp_pose[:6], is_servo=True)
            self.hand.move(grasp_pose[6:])
            print("Moved to grasp pose first")
            return

        if self.hand_grasp_progress < 3.0:
            self.hand_grasp_progress = min(3.0, self.hand_grasp_progress + self.hand_grasp_speed)

            # Stage 3: grasp -> squeezed (full pose)
            stage_progress = self.hand_grasp_progress - 2.0
            current_full_pose = grasp_pose + (squeezed_pose - grasp_pose) * stage_progress
    ...
    def _execute_lift(self):
        """Lift 실행 - aftergrasp_traj를 따르거나 lift_distance만큼 올라감"""
        # Use aftergrasp trajectory if available (full DOF trajectory)
        if self.aftergrasp_traj is not None:
            # 이미 trajectory 완료
            if self.aftergrasp_progress >= len(self.aftergrasp_traj):
                print("Aftergrasp trajectory complete!")
                return

            current_qpos = self.robot.get_data()['qpos'].copy()

            # 현재 목표 waypoint
            target_idx = int(self.aftergrasp_progress)
            target_qpos = self.aftergrasp_traj[target_idx]

            # Move hand to target hand pose
            self.hand.move(target_qpos[6:])

            # Calculate distance for arm
            distance = np.linalg.norm(current_qpos - target_qpos[:6])
            ...
            if distance > 0.01:  # Not reached yet
                ...
                # Reached current waypoint, move to next
                self.aftergrasp_progress += 1
                progress_pct = (self.aftergrasp_progress / len(self.aftergrasp_traj)) * 100
                if self.aftergrasp_progress >= len(self.aftergrasp_traj):
                    print(f">>> Aftergrasp trajectory complete! (100%)")
                else:
                    # Print every waypoint for aftergrasp (usually shorter trajectory)
                    print(f"Aftergrasp progress: {target_idx}/{len(self.aftergrasp_traj)} ({progress_pct:.1f}%)")

        else:
            # Fallback to simple lift with lift_distance
    ...
    def _execute_place(self):
        ...
        # State 1: Release hand
            ...
            start_pose = self.predefined_hand_traj['start'][0,:]  # Extract first row (12 DOF)
            self.hand.move(start_pose[6:])  # Move hand to open position
    ...
    def _handle_hand_grasp_control(self, pressed_buttons):
        """Handle hand grasp trajectory execution with 4 stages (full pose: arm + hand)"""
        if self.predefined_hand_traj is None:
            return

        # Extract full poses (12 DOF: arm 6 + hand 6)
        start_pose = self.predefined_hand_traj['start'][0,:]
        pregrasp_pose = self.predefined_hand_traj['pregrasp'][0,:]
        grasp_pose = self.predefined_hand_traj['grasp'][0,:]
        squeezed_pose = self.predefined_hand_traj['squeezed'][0,:]
        ...
        while self.running:
            ...
            if pressed_buttons:
                ...
                # Grasp 버튼 체크
                    ...
                    self._execute_grasp()
                # Squeeze 버튼 체크
                elif 'squeeze' in pressed_buttons:
                    self._execute_squeeze()
                # Lift/Aftergrasp 버튼 체크
