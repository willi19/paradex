...
contact_cmap = lambda levels, thres=1.0: [f"rgb({int(255 * (1 - x))},{int(255 * (1 - x))},{int(255 * (x))})" if x < thres else
                                         f"rgb({int(0)},{int(0)},{int(255)})" for x in levels.tolist()]
...
def plot_point_cloud_cmap_sharp(pts, color_levels=None):
    import numpy as np

    # 마스크 생성: color_levels > 0.5인 포인트만 필터링
    if color_levels is not None:
        mask = color_levels > 0.5
        pts = pts[mask]
        color_levels = np.ones_like(color_levels[mask]) 
