...
serial_num = 22640993
keypoint_path = os.path.join(f"{shared_dir}/intrinsic/{serial_num}/keypoint")
keypoint_file = os.listdir(keypoint_path)[-1]

keypoint = np.load(os.path.join(keypoint_path, keypoint_file))

img = np.zeros((1536, 2048, 3), dtype=np.uint8)
for kypt in keypoint:
    draw_charuco_corners_custom(img, kypt, BOARD_COLORS[0], 5, -1)
    cv2.imshow("int", img)
    cv2.waitKey(0)