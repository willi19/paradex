...
from paradex.object_detection.process_module import process_object
import argparse
...
try:
    ...
    while True:
        ...
        tasks = []
        for obj_name in args.obj_names:
            process_args = cur_state, obj_name, scene, obj_dict_bucket, img_bucket, args, \
                oneview_dir, optimization_dir, inliers_threshold, loss_thres, sampled_obj_verts
            tasks.append(process_args)
            
        with ThreadPoolExecutor(max_workers=4) as executor:
            futures = [executor.submit(process_object, t) for t in tasks]
            for future in as_completed(futures):
                obj_name, matchingset_list = future.result()
                for _, matchingset in enumerate(matchingset_list):
        ...
        for matchingset_key, matchingset in ttl_matchingset_list.items(): 
            ...
            if reoptim:
                ...
                print(f'loss:{min_loss} optim_ouptput:{optim_output}')
        ...
        print(f"Found {len(ttl_output_dict)} objects : { ','.join([key for key in ttl_output_dict])}")
