...
from paradex.object_detection.obj_utils.io import makevideo, get_binary_mask
...
def compute_filtered_mean(diff, filter, percentile_thres=80):
    thres = np.percentile(diff[filter],percentile_thres)
...
def optimize_with_initial_sRt(input_data):
    ...
    model_inputs, rotation_id, scaled, default_scale, rotation_candidate, initial_translate, learning_args, args, object_output_dir, frame_numb = input_data
    scene, obj_dict, tg_cams, obj_mask_stackted_t, obj_masked_rgb_stacked_t, gt_combined_mask, object_final_output_dir, cam_filters, robot_rendered_silhouette = model_inputs
    ...
    if cam_filters is not None:
        rendered_rgb_batched = rendered_rgb_batched[cam_filters]
        rendered_silhouette_batched = rendered_silhouette_batched[cam_filters]

    init_sil_mse = nn.functional.mse_loss(obj_mask_stackted_t, rendered_silhouette_batched).item()
    init_rgb_mse = nn.functional.mse_loss(obj_masked_rgb_stacked_t, rendered_rgb_batched).item()
    ...
    if frame_numb == 0:
        ...
        cv2.imwrite(str(object_final_output_dir/('rotation_%s_rgb_initial.png'%(rotation_id))), \
                    np.vstack([make_grid_image(obj_masked_rgb_stacked_t, 1, batch_num).detach().cpu().numpy()*255,
        ...
        cv2.imwrite(str(object_final_output_dir/('rotation_%s_sil_initial.png'%(rotation_id))),commbined_sil*255)
        print(f'Initial {rotation_id}: rgb_loss {init_rgb_mse} sil_loss {init_sil_mse}')
    ...
    for nit in range(iter_numb):
        ...
        transformed_obj_dict = get_transformed_obj(obj_dict, optim_scale, optim_R, optim_t)
        rendered_rgb_batched, rendered_silhouette_batched = scene.batch_render(transformed_obj_dict, render_rgb = args.use_rgb)
        if cam_filters is not None:
            rendered_rgb_batched = rendered_rgb_batched[cam_filters]
            rendered_silhouette_batched = rendered_silhouette_batched[cam_filters]
        ...
        if args.use_sil:
            ...
            if robot_rendered_silhouette is not None:
                sil_mse = ((~robot_rendered_silhouette.unsqueeze(dim=-1)).to(torch.float32)*sil_mse)
        ...
        if args.use_rgb:
            ...
            if robot_rendered_silhouette is not None:
                rgb_mse = ((~robot_rendered_silhouette.unsqueeze(dim=-1)).to(torch.float32)*rgb_mse)
    ...
    # if frame_numb==0:
    #     # Final Visualization
    cv2.imwrite(str(object_final_output_dir/('rotation_%s_%05d_rgb_final.png'%(rotation_id, frame_numb))), \
                np.vstack([make_grid_image(obj_masked_rgb_stacked_t, 1, batch_num).detach().cpu().numpy()*255,
                            make_grid_image(rendered_rgb_batched, 1, batch_num).detach().cpu().numpy()*255]))
    ...
    commbined_sil[gt_combined_mask[...,0]>0] += [1,0,0]
    commbined_sil[renderd_combined_mask[...,0]>0] += [0,1,0]
    cv2.imwrite(str(object_final_output_dir/('rotation_%s_%05d_sil_final.png'%(rotation_id, frame_numb))),commbined_sil*255)

    results = []
    for cidx, cam_id in enumerate(tg_cams):
        bgr_img = scene.get_image(cam_id, 0) # width, height = 512, 384
        mask=rendered_silhouette_batched[cidx].detach().cpu().numpy()[...,0]
        overlaid = overlay_mask(bgr_img, mask=(mask>0))
        results.append(overlaid)
        
    cv2.imwrite(str(object_final_output_dir/('%05d_obj_%s_rendered.png'%(frame_numb, rotation_id))),make_grid_image_np(np.array(results), 4,6))

    pickle.dump(result_dict, open(object_final_output_dir/f'obj_output_after_optim_total_{rotation_id}.pickle','wb'))
    json.dump({'rgb_loss':rgb_mse, 'sil_loss':sil_mse, 'init_rgb_loss':init_rgb_mse, 'init_sil_loss':init_sil_mse}, open(object_final_output_dir/f'obj_final_loss_{rotation_id}.json','w'))

    end_time = time.time()
    print(f'Optimization for one candidate ended in {end_time-start_time}')
    return result_dict, rgb_mse, sil_mse, init_rgb_mse, init_sil_mse
...
def get_gt_data(scene, mask_root, cam_ids, device, computed_mask:dict=None, fidx=0):
    ...
    for cam_id in cam_ids:
        ...
        rgb_img_np = scene.get_image(cam_id, fidx=fidx)# cv2.cvtColor(scene.get_image(cam_id, fidx=fidx), cv2.COLOR_BGR2RGB)
    ...
    ed_time = time.time()
    print(f"Getting GT Data {ed_time-st_time} sec")
...
def load_obj_masks(org_scene:Scene, reload_mask = False, debug=False, confidence=0.002):
    ...
    if reload_mask:
        ...
        for cam_id in org_scene.cam_ids:
            ...
            ed_time = time.time()
            print(f"yolo: one_frame takes {ed_time-st_time}")
        ...
        if debug:
            cv2.imwrite(str(PROJECT_DIR/'debug'/f'debug_grid_{obj_name}.png'), make_grid_image_np(np.array(results_img), 4,6))
        ...
        for cam_id in org_scene.cam_ids:
            ...
            if check_mask(mask):
                ...
                overlayed = overlay_mask(bgr_img, mask>0, alpha=0.3)
                results_img.append(overlayed)
                # cv2.imwrite('test.png', cv2.cvtColor(overlayed, cv2.COLOR_BGR2RGB))
        
        if debug:
            cv2.imwrite(str(PROJECT_DIR/'debug'/f'debug_premask_grid_{obj_name}.png'), make_grid_image_np(np.array(results_img), 4,6))
...
def parse_and_crop(obj_mask_t_dict, obj_masked_rgb_t_dict, img_L=112):
        # Parse GT mask
    tg_rgbs, tg_sils = [], []
    ...
    for cam_id in obj_masked_rgb_t_dict:
        ...
        tg_rgb = crop_and_resize_by_mask(obj_masked_rgb_t_dict[cam_id][0].detach().cpu().numpy()*255,  \
                                        obj_mask_t_dict[cam_id][0].detach().cpu().numpy()*255, img_L)
...
def render_and_compute_loss_all_cams(obj_dict, scene, rot_6d_t, initial_translate, default_scale,\
    ...
    '''
        mask_filter: compare amount of mask and silhouette
        rgb_filter or sil_filter : drop lowest 20% of RGB and Silhouette Foreground Difference
    '''
    if turn_on_filter:
        ...
        mask_filter = mask_percentage>0.7
...
def render_and_compute_loss_all_cams_dino(obj_dict, scene, rot_6d_t, initial_translate, default_scale,\
                                    tg_rgbs_full, tg_sils_full,\
                                    tg_features, dino_module=None, use_local_feature=False, image_size=112,\
                                    turn_on_filter=False, device='cuda'):
    
    assert dino_module is not None, 'You should use dino model'
        
    trasnformed_obj_dict = get_transformed_obj(obj_dict, default_scale, rot_6d_t,\
                                                torch.tensor(initial_translate).to(device).float(), )
    rendered_rgb_batched, rendered_silhouette_batched = scene.batch_render(trasnformed_obj_dict)
    N_cam = tg_features.shape[0]
    ...
    rendered_rgbs_tensor = torch.tensor(rendered_rgbs, device=device)/255
    rendered_features = dino_module.extract_feature(rendered_rgbs_tensor)

    if not use_local_feature:
        feature_diff = torch.linalg.norm(rendered_features['x_norm_clstoken']-tg_features['x_norm_clstoken'], axis=1).detach().cpu().numpy()
    else:
        feature_diff = torch.linalg.norm(rendered_features['x_norm_patchtokens'].reshape(N_cam,-1)-\
                                           tg_features['x_norm_patchtokens'].reshape(N_cam,-1), axis=1).detach().cpu().numpy()
    combined_filter = np.ones(feature_diff.shape[0], dtype=np.bool_)

    if turn_on_filter: # compute rendered target mask and amount of rendered silhouette to filter out cam with large occlusion.
        rendered_rgbs_full = rendered_rgb_batched.detach().cpu().numpy()*255
        rendered_sils_full = rendered_silhouette_batched.squeeze().detach().cpu().numpy()*255
        ...
        combined_filter = np.logical_or(rgb_filter, sil_filter)

        if len(rgb_filter) <= 2 or len(sil_filter) <= 2: # If Candidate Fails
            feature_loss = 1000
        else:
            feature_loss, combined_filter = compute_filtered_mean(feature_diff, combined_filter)
    else:
        feature_loss = np.mean(feature_diff)

    return rendered_rgbs, rendered_rgbs_full, rendered_sils, rendered_sils_full, \
            rgb_diff_full, sil_diff_full, combined_filter, feature_loss
