...
class AravisCaptureTests(unittest.TestCase):
    ...
    def test_partial_reconcile_skips_camera_that_remains_link_local(self):
        nic = NicSubnet("enp5s0", "11.0.1.1", ipaddress.ip_network("11.0.1.0/24"))
        good = CameraRecord("cam-good", "FLIR-cam-good", "11.0.1.100", "2c:dd:a3:7d:a6:9c")
        stuck = CameraRecord("cam-stuck", "FLIR-cam-stuck", "169.254.1.2", "2c:dd:a3:7d:a6:9d")
        addressing = CameraAddressing([nic])
        addressing._seen = {good.serial: good, stuck.serial: stuck}
        addressing.discover = MagicMock(return_value=addressing.seen)
        addressing.force_ip = MagicMock()

        with patch("paradex.io.camera_system.aravis_addressing.time.sleep"), patch.object(
            addressing, "_verify", return_value=True
        ), self.assertLogs("paradex.io.camera_system.aravis_addressing", level="WARNING") as logs:
            usable = addressing.reconcile(
                [good.serial, stuck.serial],
                allow_partial=True,
            )

        self.assertEqual(usable, [good.serial])
        self.assertTrue(any(stuck.serial in message for message in logs.output))
