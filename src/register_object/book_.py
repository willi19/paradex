...
goal_marker = [("1", "xz"), ("2", "xz"), ("3", "xz"), ("4", "xz")]
goal_position = get_book_goal_position(img_dict, goal_marker)
...
for i, serial_num in enumerate(serial_list):
    ...
    draw_aruco(debug_img_dict[serial_num], robot_2d, list(floor_cor_3d.keys()))

for gi in goal_marker:
    mesh_ = copy.deepcopy(mesh)
    mesh_.apply_transform(goal_position[gi]) 
    print(goal_position[gi])
    frame, mask = project_mesh_nvdiff(mesh_, renderer)
    mask = mask.detach().cpu().numpy()[:,:,:,0].astype(np.bool_)
    
    for i, serial_num in enumerate(serial_list):
        overlay_mask(debug_img_dict[serial_num], mask[i], np.array((255,0, 0)), 0.7)
    
for i, serial_num in enumerate(serial_list):
    cv2.imwrite(os.path.join(shared_dir, f"object/marker_offset/book/{index}/debug/{serial_num}.png"), debug_img_dict[serial_num])
