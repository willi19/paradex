...
# 박스 크기 (mm 단위)
width = 23.4 / 100  # x 축 (cm) 
height =  5.5 / 100 # y 축 (cm)
depth = 21.0 / 100    # z 축 (cm)

# trimesh로 박스 생성 (크기를 지정)
# mesh.export(os.path.join(rsc_path, "object", "book", "book.obj"))
root_dir = os.path.join(shared_dir, "obj_register", "box")
...
for index in os.listdir(root_dir):
    ...
    obj_T[:3, 1] = y_dir
    obj_T[:3, 2] = z_dir
    obj_T[:3, 3] = middle - depth/2 * z_dir
    ...
    os.makedirs(os.path.join(shared_dir, f"RSS2026_Mingi/marker_offset/box/"), exist_ok=True)
    np.save(os.path.join(shared_dir, f"RSS2026_Mingi/marker_offset/box/{index}.npy"), marker_offset)
