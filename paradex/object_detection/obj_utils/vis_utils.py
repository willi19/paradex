...
from paradex.object_detection.obj_utils.geometry import transform_by_extrinsic
import trimesh
...
MESH_DIR = os.path.join(os.environ['NAS_PATH'], 'object_6d', 'data', 'mesh')
...
def read_frame(video_path, frame_number=0, save = False):
    '''
...
def crop_and_resize_by_mask_tensor(image: torch.Tensor,
                                  mask: torch.Tensor,
                                  output_size: int) -> torch.Tensor:
    """
    Args:
        image (H x W x C): 배치 RGB 이미지 텐서
        mask  (H x W): 마스크 텐서 (bool 또는 0/1)
        output_size (int): 최종 출력 크기 (output_size x output_size)

    Returns:
        output (output_size x output_size x C): 출력 이미지들
    """
    H, W, C = image.shape
    ...
    if ys.numel() == 0:
        return torch.zeros((output_size, output_size, C), dtype=image.dtype, device=image.device)
    ...
    if width > height:
        delta = width - height
        y_min = max(0, y_min - delta // 2)
        y_max = min(H - 1, y_max + (delta - delta // 2))
    else:
        delta = height - width
        x_min = max(0, x_min - delta // 2)
        x_max = min(W - 1, x_max + (delta - delta // 2))

    cropped = image[y_min:y_max+1, x_min:x_max+1]  # h x w x C
    cropped = cropped.permute(2, 0, 1).unsqueeze(0).float()  # 1 x C x H x W
    resized = F.interpolate(cropped, size=(output_size, output_size), mode='bilinear', align_corners=False)
    resized = resized.squeeze(0).permute(1, 2, 0)  # output_size x output_size x C
...
def pad_bbox(xyxy_list, width, height, percentage=0.1):
    # x ~ width
    # h ~ height
    ...
    for xyxy in xyxy_list:
        ...
        pad_x = int((x_max-x_min)*percentage)
        pad_y = int((y_max-y_min)*percentage)

        padded_xyxy_list.append((max(x_min-pad_x, 0),max(y_min-pad_y,0),min(x_max+pad_x, width),min(y_max+pad_y, height)))

    return padded_xyxy_list