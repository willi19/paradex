...
class CMapAdam:
    ...
    def reset(self, contact_map_goal, running_name, energy_func_name):
        ...
        self.q_current = torch.zeros(self.num_particles, len(self.handmodel.revolute_joints),
                                     device=self.device)
        ...
        self.q_current = self.init_random_scale * torch.rand_like(self.q_current) * (self.q_joint_upper - self.q_joint_lower) + self.q_joint_lower
    ...
    def compute_energy_euclidean_dist(self):
        ...
        z_norm = F.relu(self.q_current - self.q_joint_upper) + F.relu(self.q_joint_lower - self.q_current)
    ...
    def compute_energy_align_dist(self):
        ...
        if self.robot_name == 'robotiq_3finger':
            ...
            z_norm = torch.abs(self.q_current - q_joint_mid).sum(dim=1)
