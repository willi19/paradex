...
## Usage
Both run **on the capture PCs** (not the main PC). They are typically launched
over SSH by the main-PC orchestrators (`run_script` / `remote_camera_controller`),
...
## Inputs & Outputs
- `server_daemon.py`: no CLI args. Listens on ZMQ ports (ping 5480, command 5482,
  connection 5483). Capture results are written to disk by the `CameraLoader`
