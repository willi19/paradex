...
def get_action(palm_pose, last_qpos=None):
    if last_qpos is None:
        success = False
        while not success:
            qpos, success = robot.solve_ik(
                palm_pose,"link6", None, 1000
            )
    else:
        qpos, success = robot.solve_ik(
            palm_pose,"link6",last_qpos.copy(),1000
        )
        while not success:
            qpos, success = robot.solve_ik(
                palm_pose,"link6", None, 1000
    ...
    return np.concatenate([qpos[:6], np.zeros(12)]).astype(np.float32)
...
print("asdfasdf", traj[0])
init_action = get_action(traj[0])
...
for i in range(len(traj)):
    ...
    action = get_action(traj[i], last_q)
    last_q = action.copy()
