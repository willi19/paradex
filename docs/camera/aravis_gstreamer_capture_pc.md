...
### 기존 `flir_env` Conda 환경에 연결

Ubuntu 20.04의 system Python과 `flir_env`가 모두 Python 3.8일 때만 시스템
`python3-gi`를 Conda 환경에 연결할 수 있다. 먼저 반드시 버전을 비교한다.
...
둘 다 `Python 3.8.x`일 때 다음을 한 번 실행한다. Conda 환경 설정에 저장되므로
이후 `flir_env`를 활성화할 때마다 별도로 `export`할 필요가 없다.
...
```bash
conda activate flir_env
conda env config vars unset GI_TYPELIB_PATH PYTHONPATH
conda deactivate
conda activate flir_env
```
