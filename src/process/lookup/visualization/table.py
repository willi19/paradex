...
demo_path = os.path.join(shared_dir, "capture","lookup", "pringles", hand_name)
...
for demo_name in demo_name_list:
    for type in ["pick", "place"]:
        ...
        height_offset = np.load(os.path.join(demo_path, str(demo_name), f"obj_T.npy"))[0, 2, 3]
        ...
        for i in range(T):
            wrist_pos[i] = obj_pos[0] @ wrist_pos[i] @ LINK2WRIST
...
while True:
    ...
    while cur_env_idx < len(env_list) and len(cur_idx) < max_env_num:
        ...
        sim.set_videopath(env_name, os.path.join("capture","lookup", "pringles", hand_name))
